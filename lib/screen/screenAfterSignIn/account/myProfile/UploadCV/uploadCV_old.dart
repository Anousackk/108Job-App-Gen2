...
class _UploadCVState extends State<UploadCV> {
      ...
      await showDialog(
        ...
        builder: (context) {
          return NewVer2CustAlertDialogSuccessBtnConfirm(
            ...
            contentText: "upload cv".tr + " " + "successfully".tr,
            textButton: "ok".tr,
            press: () {
              Navigator.pop(context);
              Navigator.pop(context);
  ...
  checkCvStatusDisplayScreen() async {
    setState(() {
      if (widget.cv != null) {
        _isCVStatus = false;
        pressTapCvAndVipoInfomation("CV");
      } else if (widget.cv == null && widget.vipoCV == null) {
        _isCVStatus = false;
        pressTapCvAndVipoInfomation("CV");
      } else {
        _isCVStatus = true;
        pressTapCvAndVipoInfomation("VIPO CV");
      }
    });
  }

  pressTapCvAndVipoInfomation(String status) {
