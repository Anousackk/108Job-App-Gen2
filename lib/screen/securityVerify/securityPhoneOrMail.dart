...
class SecurityPhoneNumberOrEmail extends StatefulWidget {
  const SecurityPhoneNumberOrEmail({Key? key, this.addPhoneNumOrEmail})
      : super(key: key);
  final addPhoneNumOrEmail;

  @override
  State<SecurityPhoneNumberOrEmail> createState() =>
      _SecurityPhoneNumberOrEmailState();
}

class _SecurityPhoneNumberOrEmailState
    extends State<SecurityPhoneNumberOrEmail> {
                          ...
                          press: () {
                            if (formkey.currentState!.validate()) {
                              if (_isToken != "") {
                                ...
                                print("work api request otp code");
                                requestOTPCode();
                              }
                            }
                          },
  ...
  requestOTPCode() async {
    ...
    } else if (res['message'] == "Mobile does not exist") {
      await showDialog(
        context: context,
        builder: (context) {
